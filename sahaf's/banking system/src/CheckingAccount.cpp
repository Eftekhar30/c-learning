...
CheckingAccount::CheckingAccount() 
    : Account(), dataFile(""), size(nullptr), customers(nullptr), 
      accounts(nullptr), feeCharge(0.50) {}

CheckingAccount::CheckingAccount(const std::string& filename, int* sizePtr)
    : Account(), dataFile(filename), size(sizePtr), feeCharge(0.50) {
    
    customers = new Customer*[*size];
    accounts = new CheckingAccount*[*size];
    ...
    while (std::getline(file, line) && index < *size) {
        ...
        accounts[index] = new CheckingAccount(fname, lname, accNum, bal, ctype, mail, phone);
        index++;
    }
    file.close();
}

CheckingAccount::CheckingAccount(const std::string& firstname, const std::string& lastname,
    ...
    : Account(firstname, lastname, accountNumber, balance, cardtype, email, phonenumber),
      dataFile(""), size(nullptr), customers(nullptr), accounts(nullptr), feeCharge(0.50) {}

int CheckingAccount::findAccount(int accountNum) {
    for (int i = 0; i < *size; i++) {
...
void CheckingAccount::deposit(double amount) {
    ...
    if (customers[idx]->login()) {
        double netDeposit = amount - feeCharge;  // Fee deducted
        accounts[idx]->balance += netDeposit;
        std::cout << "\t\t\tFee charged: $" << feeCharge << std::endl;
        std::cout << "\t\t\tNet deposit: $" << netDeposit << std::endl;
...
void CheckingAccount::withdraw(double amount) {
    ...
    double totalAmount = amount + feeCharge;
    if (accounts[idx]->balance < totalAmount) {
        std::cout << "\t\t\tInsufficient balance!\n";
        std::cout << "\t\t\tAmount needed: $" << totalAmount << " (including $" << feeCharge << " fee)\n";
    ...
    accounts[idx]->balance -= totalAmount;
...
// updateAccount, addAccount, searchAccount - similar to SavingsAccount
void CheckingAccount::updateAccount() {
    // Implementation similar to SavingsAccount::updateAccount()
    display();
    int accountNum;
...
void CheckingAccount::addAccount(int& sizeRef) {
    ...
    do {
        ...
        file << std::left << std::setw(20) << fname << std::setw(20) << lname
             << std::setw(20) << uname << std::setw(20) << pword << std::setw(20) << accNum
             << std::setw(20) << bal << std::setw(20) << ctype << std::setw(30) << mail
...
void CheckingAccount::searchAccount() {
    int accountNum;
    std::cout << "\t\t\tEnter account number: "; std::cin >> accountNum;
    int idx = findAccount(accountNum);
    if (idx == -1) { std::cout << "\t\t\tAccount not found!\n"; return; }
...
void CheckingAccount::shiftAccountsLeft(int index) {
...
void CheckingAccount::deleteAccount(const std::string& filename, int& sizeRef) {
    if (!admin.login()) return;
    display();
    int accountNum;
    std::cout << "\t\t\tEnter account number to delete: "; std::cin >> accountNum;
    int idx = findAccount(accountNum);
    if (idx == -1) { std::cout << "\t\t\tAccount not found!\n"; return; }
...
void CheckingAccount::saveAccountData() {
    std::ofstream file(dataFile);
    if (!file.is_open()) return;
    for (int i = 0; i < *size; i++) {
        file << std::left << std::setw(20) << accounts[i]->firstname << std::setw(20) << accounts[i]->lastname
             << std::setw(20) << customers[i]->getUsername() << std::setw(20) << customers[i]->getPassword()
             << std::setw(20) << accounts[i]->accountNumber << std::setw(20) << accounts[i]->balance
             << std::setw(20) << accounts[i]->cardtype << std::setw(30) << accounts[i]->email
...
void CheckingAccount::display() const {
    ...
    std::cout << std::left << std::setw(10) << "Type" << std::setw(20) << "Name"
              << std::setw(15) << "Account No" << std::setw(20) << "Username"
              << std::setw(15) << "Card Type" << std::setw(30) << "Email"
              << std::setw(15) << "Phone" << std::setw(15) << "Balance" << std::endl;
    std::cout << std::string(150, '=') << std::endl;
    
    for (int i = 0; i < *size; i++) {
        std::cout << std::left << std::setw(10) << "Checking"
...
CheckingAccount::~CheckingAccount() {
    ...
    if (accounts) {
        for (int i = 0; i < *size; i++) delete accounts[i];
        delete[] accounts;
    }
}